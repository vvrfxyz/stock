...
权威实现是 `main.py` 的 `build_scheduled_update_steps()`；完整 step list 由测试
`tests/test_scheduled_update.py::test_scheduled_update_runs_daily_tasks_every_day` /
`test_scheduled_update_staggers_weekly_tasks` / `test_scheduled_update_staggers_monthly_tasks` 锁定。
