...
输出是人工甄别清单，不写库。仅"阻塞 active-only 唯一索引迁移的硬冲突"以非零
退出码接调度告警；"需人工甄别但不阻塞"的 advisory 发现退出 0（只在日志显著
输出，不把每日调度打红——2026-07-07 调整）。
...
def resolve_exit_code(blocking: int, advisory: int) -> int:
    """汇总计数并给出退出码。

    只有阻塞迁移的硬冲突才非零退出；advisory（需人工甄别但不阻塞）退出 0，
    仅在日志显著输出——否则每周审计步在 pipeline_task_runs 记 FAILED、
    systemd OnFailure 假告警（2026-07-07 调整）。
    """
    logger.info("--- 身份对账汇总 ---")
    ...
    if advisory > 0:
        logger.warning(
            "⚠️ 发现需人工甄别的存量身份异常: {} 组（advisory 不阻塞调度，exit 0），详见上方样例。",
            advisory,
        )
        return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            return resolve_exit_code(blocking, advisory)
