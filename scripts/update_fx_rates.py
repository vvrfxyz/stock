...
# CCUSMA02ILM618N（OECD 经 FRED）：1 USD = rate ILS，月频（当月日均值，rate_date 为月首），
# 1957 年起——ECB 的 EUR->ILS 序列 2011-01-03 才开始，2007-2009 的以色列跨上市分红
# （ITRN/CEL）靠此序列折算；月频行需要读取层放宽 staleness（见 utils/fx_rates
# FALLBACK_STALENESS_OVERRIDES）。FRED 无日频 ILS 序列（DEXISUS 不存在）。
FRED_FX_SERIES: dict[str, tuple[str, str]] = {
    "DEXTAUS": ("USD", "TWD"),
    "CCUSMA02ILM618N": ("USD", "ILS"),
}
