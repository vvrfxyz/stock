...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cutoff", default=DEFAULT_CUTOFF.isoformat(),
                        help="只导入早于该日(YYYY-MM-DD)的事件，该日起归 live sync；"
...
def resolve_events(
    ...
    for row in rows:
        ...
        if len(hits) == 1:
            by_security[next(iter(hits))].append(row)
