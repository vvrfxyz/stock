...
R3 非 E 前缀行（P 前缀混有真分拆价值因子与被错标的真实拆股，语义由裁决层用
allowlist 可选第 6 列 adjustment_type_override 逐事件定：'spinoff_pseudo_split'
= 分拆价值因子，价格因子链消费、research/market_cap 股本前滚同日抑制；留空 =
真实拆股透传 vendor adjustment_type，股本照常前滚）与 R10 冲突组（只放行点名
成员，其余照旧隔离；同组点名矛盾比例直接报错）。未点名的行行为不变。
...
def load_split_rows(path: Path, stats: Counter, quarantine: Counter, detail: list[dict],
                    allowlist: dict[str, dict] | None = None) -> list[dict]:
    """parquet -> 规范化拆股行。R3：非 E 前缀 id 是 spinoff 伪拆分，整体隔离。

    allowlist 模式例外（2026-07 round-2 裁决）：allowlist 显式点名的非 E 行放行。
    R3 的"P 前缀=spinoff"是抽样启发式，实测 P 行混有真实拆股（ANDE 3-for-2、
    iShares 2005 系列、UCAR 100:1 等，vendor adjustment_type 也无法区分——GE/GEV
    分拆同样标 forward_split），故语义由裁决层逐事件定：allowlist 第 6 列
    adjustment_type_override='spinoff_pseudo_split' 的行打抑制标记（价格因子链
    照常消费，research/market_cap 股本前滚同日抑制），留空的行透传 vendor 值
    （真实拆股，股本照常前滚）。未点名的非 E 行照旧整体隔离。"""
    import pandas as pd
    ...
    for rec in frame.itertuples(index=False):
        ...
        if not event_id.startswith("E"):
            if allowlist is not None and event_id in allowlist:
                override = allowlist[event_id].get("adjustment_type_override")
                ...
                rows.append({
                    ...
                    "split_to": split_to,
                    "adjustment_type": override or _to_str(rec.adjustment_type),
                })
                continue
...
def parse_adjudicated_allowlist(lines) -> dict[str, dict]:
    ...
    可选第 6 列 adjustment_type_override（round-2：仅允许 'spinoff_pseudo_split'，
    用于把 vendor 错标为 forward_split 的分拆价值因子打上抑制标记——见
    research/market_cap.load_split_events 的股本前滚同日抑制契约；留空=透传 vendor 值）。
    ...
    extended = expected + ["adjustment_type_override"]
    header_cols = header.rstrip("\n").split("\t") if header is not None else []
    if header_cols not in (expected, extended):
        raise ValueError(f"allowlist 表头异常（期望 {expected} 或 {extended}）。")
    ...
    for line in it:
        ...
        event_id, security_id, ticker, ex_date, kind = parts[:5]
        override = (parts[5] or None) if n_cols == 6 else None
        if kind not in ("dividend", "split"):
            raise ValueError(f"allowlist kind 非法: {kind}（须 dividend/split）")
        if override is not None and override != "spinoff_pseudo_split":
            raise ValueError(
                f"allowlist adjustment_type_override 非法: {override}（只允许 spinoff_pseudo_split）")
        entry = {
            "security_id": int(security_id),
            "ticker": ticker.upper(),
            "ex_date": date.fromisoformat(ex_date),
            "kind": kind,
            "adjustment_type_override": override,
        }
...
def _allowlist_report_path(path: Path) -> Path:
    """allowlist 模式的报告改道路径：stem 加 `_allowlist` 后缀。

    allowlist 模式绕过 resolve_events，out_of_tenure/ambiguous 明细永远不会再生成；
    若仍写默认路径，会用残缺版本覆盖全量导入产出的隔离明细——那是
    adjudicate_polygon_orphans 的 --quarantine-detail 输入且被 gitignore（只存在
    于磁盘）。mismatch 报告同理（全量 run 的 R13 人工裁决队列不可被 allowlist run
    的子集覆盖）。故三份报告在 allowlist 模式一律改道，绝不触碰全量 run 的工件。
    """
    return path.with_name(f"{path.stem}_allowlist{path.suffix}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # allowlist 模式绕过 resolve_events，隔离/冲突报告只含残缺子集——改道写
        # `*_allowlist.tsv`，绝不覆盖全量导入产出的裁决输入工件（见 _allowlist_report_path）。
        redirect = _allowlist_report_path if allowlist is not None else (lambda p: p)

        report_path = redirect(project_root / args.quarantine_report)
        ...
        detail_path = redirect(project_root / args.quarantine_detail_report)
        ...
        mismatch_path = redirect(project_root / args.mismatch_report)
