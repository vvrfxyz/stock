...
| **composite_v2（low_vol + high_52w 残差 + size + operating_profitability，0.5 中性填补）** | 日线+PIT 市值+XBRL | 2012+ 同窗同口径三腿对比：IC h5 .0274/t=4.38（v1 .0265/low_vol .0274）、q5 净 Sharpe h21 **0.734 vs low_vol 单干 0.743**——§7 双条件 FAIL（不严格优于最优单成分）；散户终审 fixed 双口径 FAIL（40bps t=1.458）；**measured 实测价差重审（2026-07-09）：alpha t=2.1296、中位超额 +1.4pp——判据 PASS，但 q5 成员价差覆盖 64%<70% 资格线，结论标注"覆盖不足、仅供参考"**。四腿 measured 谱系：v2 2.13(PASS*)/v1 1.97/OP 1.72/EY 皆 FAIL——无廉价全翻案 | **调味料 → 待资格线确认的翻案候选（2026-07-09）；纸面组合冻结直至覆盖率过线** | measured 重审是 §6 预注册路线（判据零改动、只换实测成本输入——q5 实测单边中位 8.9bps vs fixed 单边 40bps 高估 4 倍余）。**t 1.46→2.13 跨线且四腿只翻 v2 一个（选择性翻案增可信度）**，但 36% q5 成员吃 40bps fallback——覆盖不足下 PASS 可能是价差数据在场票的选择偏差，**资格线防线（预注册）拦住启动**。解锁路径=工程题：查 minute_loader join 是否漏接可用价差/63 日中位窗对新票滞后，覆盖抬过 70% 后重跑同口径 trial 终裁 | v2 收官链报告×5、measured 报告×4、study 行×6（2026-07-08/09） |
