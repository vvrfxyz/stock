...
| **earnings_yield（盈利收益率，NI_TTM/公司级市值）** | XBRL+PIT 市值 | 2012+ 正式重跑（P2 退市注入新口径）：IC .0126-.0269，**nw_t h1 5.59 / h5 3.88 / h10 3.64 / h21 3.29 全过动态阈值且 IC 随 horizon 递增**；三关对照全过（\|size 衰减 24%、\|low_vol 53%、\|high_52w 17%，与 size 秩相关仅 0.200）；栖息地=小盘独占（桶内 IC 小盘 .0241/t=4.89、中盘 1.6、大盘 0.5）；size 中性化部署检验 FAIL（桶匹配基准 alpha 0.7-0.9%/年 t≈0.4） | **排序可用、部署待散户复审终裁（2026-07-08）** | 项目至今最厚的非价格因子：三关全过=独立信息维度（反向 size\|ey 仅衰减 14%，与价格族正交），IC 期限结构适配月频。但栖息地与 vol 族同型（小盘独占），size 中性整分位无 alpha——变现出口收窄到"小盘桶内选股"一条路，retail_reality 双口径复审进行中。partial 为序列级近似，正式结论需逐日截面回归残差确认（预注册尾注） | wave-12 前哨；trial 49f0d58b2957(探索)+2012 正式 trial、factor_correlation/size_neutral 报告 2026-07-08 |
