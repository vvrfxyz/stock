...
| **wave-12 基本面族四因子**（gross_profitability / accruals / operating_profitability / asset_growth，预注册 docs/wave12_fundamental_hypotheses.md） | XBRL 2012+ | **operating_profitability（EBIT/AT）h1 t=7.21 全 horizon 3.82-7.21 全过**（IC .016-.031 随期限递增）；gross_profitability 仅 h1 t=3.87 过（h5-21 衰减 1.4-2.2）；**accruals IC 为正 t≤2.63（预注册方向为负——符号反）**；asset_growth \|t\|≤1.45 符号乱 | **accruals/asset_growth 死亡（2026-07-08，预注册死刑标准）；OP 全过线待三关，GP 半存活待与 OP 互 partial（预注册规则：双过线留 H1）** | 诚实先验（"存活 1-2 个、其余死"）精确命中。accruals 之死是 Sloan 效应发表后被套利殆尽的教科书案例——正显著=应计溢价翻转，预注册明说不入账为发现；asset_growth 的投资异象在 2012+ 美股横截面为零。**OP t=7.2 强得可疑——先过三关问是不是 size/vol 马甲，再问与 earnings_yield 是不是同一个东西**（构造相关：NI vs EBIT 同除 assets vs 市值） | wave-12 battery 报告×4（2026-07-08）、docs/wave12_fundamental_hypotheses.md |
