...
|---|---|---|---|---|---|
| **residual_vol（特质波动，对市场单因子残差）** | 日线 | 全 horizon 过 Bonferroni；**wave-9 终审：效应只住在小盘桶**（桶内 IC 小盘 .0343/t=4.84、中盘 t=1.4、大盘 t=-0.2） | **结案：真实但不可收割（2026-07-07）** | size 中性化部署检验 FAIL：对桶匹配基准 alpha 毛 +1.9%/年 t=1.0、25bps 后 +0.6% t=0.3——信号被 2/3 无信号桶稀释，而承载桶（~620 只小盘）的真实成本远超 10-25bps 档。**vol 族排序信息在统计上铁证如山、在经济上无出口**，全族结案 | wave-5/9 |
