...
| **wave-12 基本面族四因子**（gross_profitability / accruals / operating_profitability / asset_growth，预注册 docs/wave12_fundamental_hypotheses.md） | XBRL 2012+ | **OP（EBIT/AT）h1 t=7.21 全 horizon 全过 + 三关全过**（\|size −32%、\|low_vol −45%、\|high_52w −22%）+ **栖息地全桶显著**（小盘 t=4.91/中盘 2.65/大盘 2.05——全项目独一份）+ size 中性 alpha 1.5-1.8%/年 t 1.25-1.47（判据 FAIL 但历史最佳）；GP 仅 h1 过且 **GP\|OP=−.0015 完全吸收**；accruals 符号反；asset_growth 零 | **OP 存活至散户终审（进行中）；GP 死亡（被 OP 吸收，2026-07-08）；accruals/asset_growth 死亡（预注册死刑）** | 诚实先验命中（存活 1-2 个）。**OP 是新科最强因子且格局质变**：首个全桶显著（不困于小盘栖息地）、三关全过的独立维度；**EY\|OP=.0047（−75%）而 OP\|EY 仅 −38%——OP ⊃ EY，基本面腿由 OP 接管**（composite_v2 二选一取 OP，EY 退候补）。accruals 之死是 Sloan 套利殆尽教科书案例；partial 序列级近似尾注同前 | wave-12 battery/三关/栖息地报告（2026-07-08）、docs/wave12_fundamental_hypotheses.md |
