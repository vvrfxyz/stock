...
1. **composite_v2 覆盖率年检**（数据累积解锁，**当前唯一部署候选**）：q5 measured
   覆盖过 70% 资格线即重跑 retail_reality 终审。诚实告警（2026-07-14 追记）：
   07-09 诊断显示横截面覆盖近年**变差**（mp10 档 2020 年 68% vs 2025 年 56%——
   q5 小盘栖息地本身在变薄），纯等待不保证解锁；年检若判"结构性卡死"，
   composite_v2 做终局裁决。fs×价格族复合（fs 并入 v2 成分池的 v3 构想）
   停靠此年检一并审——五连败后不单独立项。
   **带星号纸面组合已启动（owner 2026-07-14 拍板）**：章程
   docs/paper_portfolio_charter_2026-07.md、跟踪器
   research/paper_portfolio_tracker.py、首期持仓 research/paper_portfolio/2026-07.*、
   253 定时器 stock-paper-portfolio.timer 每月 5 日 11:37 沪时自动生成（产物须
   人工确认后当月提交 git 公证）；24 个月起评 36 个月硬终点，**纸面死亡自动
   终结部署候选资格**（比年检更硬的证伪）。年检与纸面弧合并裁决。
2. ~~f_score LS 出口~~（**2026-07-14 案头结案，无需新研究**）：台账既有 trial
   （2012+ 全窗、10bps 单边）实测净 LS 年化仅 **+1~+11bps/年**（h21 档 −18bps）、
   净 Sharpe **0.01-0.05**——"全 horizon 为正"在统计上成立、经济上为零；且 q1
   空腿=小盘财务最弱段（HTB 高发），真实融券成本（30bps-5%+/年）远超 10bps
   假设，Reg-T 保证金对 2 万美元账户再砍一半仓位。**散户 LS 不可行，出口关闭；
   此裁决基于已有 trial 数据，不新增检验计数。**
