...
**institutional.py 共用加载器架构**：`institutional_breadth`、`delta_institutional_ownership`、`ownership_concentration` 三个因子共享 `research/institutional.py` 中的 `load_institutional_aggregates()`。该函数一次 SQL 完成 13F 聚合，返回一张长表事件流（含 `n_holders` / `total_value` / `total_shares` / `hhi`），PIT 口径为：只按原件（13F-HR）计可见性、排除修正件（13F-HR/A）；同一 filer 同期多份原件按口径选定唯一 accession，并对该 accession 内同证券的全部拆行求和；加载后施加 period 单调守卫，迟到申报的旧季度事件丢弃。其上 `load_institutional_holdings_panel()` 返回 4 个 PIT 宽表的 dict（`n_holders` / `total_value` / `total_shares` / `hhi`），`load_delta_institutional_ownership_panel()` 单独在事件层算好季度环比再做 as-of。注意：没有跨因子缓存——每个 builtin 的 `compute` 各自执行一次该 SQL（评估三个 13F 因子时同一重查询会跑 3 次）。

**13F 两段式可见性（2026-07）**：13F 法定截止是季末后 45 天，但单条聚合事件的 visible_date 取整组原件的 max(filing_date)，会被一家迟交机构拖垮——生产实测可见延迟 p50=98 天 / p90=246 天，叠加 staleness=200 天（锚 period）后 >25% 证券季度"生而过期"（事件可见时已超过 staleness，全程 NaN），因子面板中位截面从数据库层的 ~4-5k 跌到 557。修复：对每个 (security_id, period) 最多发两条事件——**准时批**只聚合截止日（period + 46 天，多 1 天容忍截止日顺延）前已申报的原件，visible_date = 该子集 max(filing_date)，语义是"截止日时用当时已公开的申报出首版聚合，零前视"；**终版**聚合全部原件（原行为）。两者 visible_date 相同（无迟交者）时只发终版；准时批为空（全部迟交）时也只发终版。as-of 层（merge_asof backward + period 锚）自然衔接：截止日起显示准时批值，终版到达后切换到完整值；迟交超过 staleness 的终版被 period 单调守卫丢弃或自然作废，但准时批在 staleness 窗口内始终有效。`delta_institutional_ownership` 的上季基数配对规则相应改为：取"本事件 visible_date 时点上、上季已公开的最新事件值"（visible_date <= 本事件的最新一条，准时批/终版都算候选；无候选则 NaN）——禁止上季终版可见晚于本季准时批时被用作基数（前视）。三个 builtin 因子的接口与返回形状不变。
