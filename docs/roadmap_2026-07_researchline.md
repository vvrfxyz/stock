...
>
> owner 修订（2026-07-07）：**备份挂起**（owner 在购置硬盘，到位后由 owner 亲手做，
> 不进本计划）；**密钥轮换不做**（owner 裁决）。豁免范围按字面 = PG 密码轮换 + CH 设密码；
> 源报告安全包其余 4 个非轮换子项的处置见 §8。
>
> 纪律红线（全程有效，违反即返工）：
...
- 源报告运维节点名：07-07 上午 5 次 global OOM 击杀研究进程（RSS 5.7-6.4G / 11G 主机
...
| 事项 | 时点 | 动作 | 升级条件 |
|---|---|---|---|
| OPS0 内存缓解 | W0（先于长窗作业） | run_research.sh 帽降 4.5G/5.5G；PG/CH 容器内存上限 | — |
| 日线补齐 | 07-08 跑批后 | 确认 07-06/07-07 两个交易日入库 | 连缺两日 → 升级排查 Massive 端 |
| 分钟线周度首跑 | 07-11（周六） | 人工盯 `update_minute_bars_weekly` 首个调度窗口 | 失败 → 当日修 |
| fxho rename 终裁 | 插缝半天 | QUARANTINE 终裁（sync_massive_universe 非零退出根因） | — |
| ipw/kw 重叠裁决 | 同上半天 | ipw（2021 年 5 个月）+ kw（2010 年 1 天，经审核核实为真实案例，窗口化探针永远探不到）一次性人工裁决 | — |
| 删 4 个含 API key 明文的 6 月历史日志 | 插缝（零成本件） | 253 上定位后删除，留清单记录 | 不依赖 owner 购硬盘 |
| chmod 600 activation_value.txt | 插缝（零成本件） | 两端各一次 | 同上 |
| 端口改绑 127.0.0.1/tailscale；面板关 root 密码登录 | **owner 亲手件** | 影响 owner 自己的访问方式，与备份同批处理 | 挂起至 owner 排期 |

> 安全包豁免范围说明（对抗审核发现 #0 的处置）：owner 豁免字面 = 密钥轮换（PG 密码 +
> CH 设密码）。其余 4 子项按性质分流如上——两个零成本件排进插缝，两个会改变 owner
> 访问方式的挂为 owner 亲手件。如 owner 本意豁免整包，划掉上面三行即可（留痕）。
...
| 备份 + PG/CH 密钥轮换 | owner 亲手做（硬盘到位后）；不进本计划 |
