...
要点：固定 unit 名（日志不再靠 `ls -t logs/` 猜）、`MemoryHigh=4.5G`/`MemoryMax=5.5G`
护住 11G 机器（2026-07-08 W0-OPS0 下调：07-07 实测 5 次 global OOM 都发生在 RSS
5.7-6.4G，旧帽 7G 形同虚设）。**长窗白名单抬帽**：长窗 evaluate/retail_reality
（2012+/2016+ 窗口）实测 RSS 5.7-6.4G，默认帽下必被帽杀——此类作业显式
`RESEARCH_MEMORY_HIGH=5.5G RESEARCH_MEMORY_MAX=6.5G` 拉起并错峰（同一时刻一个长窗、
避开每日跑批窗口）；短窗/诊断件走默认帽。失败经 `stock-research-notify@` 走既有
`notify_failure.sh`（logs/failures.log + 可选 webhook）。

### 容器内存硬帽（2026-07-08 W0-OPS0）

PG/CH 容器原 `Memory=0` 裸奔，已 `docker update` 在线设限（不重启）：
`stock-postgres` 2.5G、`stock-clickhouse` 4G（memory-swap 同值禁 swap 逃逸）。
...
`SYSTEM RELOAD CONFIG` 热生效。**注意 `/etc/clickhouse-server` 未挂卷：容器重建后
该文件丢失须重写**（`docker update` 的 cgroup 帽在重建后同样需要重设，除非重建时
写进 compose/run 参数）。
