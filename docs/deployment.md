...
| 数据库 | docker compose: `stock-postgres` (PG 17.5) + `stock-clickhouse`（24.12，**自 2026-07 起承载 50.6 亿行分钟线 `stock.minute_bars`，绝不可移除**；PG/CH 端口只绑定 loopback） |
...
要点：固定 unit 名（日志不再靠 `ls -t logs/` 猜）、`MemoryHigh=8G`/`MemoryMax=9G`。
2026-07-10 复核主机已扩至 19GiB RAM + 16GiB swap；PG 2.5G/CH 4G 容器帽已持久化进
compose，研究与每日跑批仍须错峰。失败经
`stock-research-notify@` 走既有 `notify_failure.sh`（logs/failures.log + 可选 webhook）。

### 容器内存硬帽（2026-07-08 W0-OPS0）

PG/CH 容器设限为 `stock-postgres` 2.5G、`stock-clickhouse` 4G
（memory-swap 同值禁 swap 逃逸），定义在 `docker-compose.yml`，容器重建后仍保留。
CH 服务端预算同步收口：`/etc/clickhouse-server/config.d/99-memory-cap.xml`
设 `max_server_memory_usage=3.2G`（约 0.8×cgroup 帽，超帽优雅拒绝查询而非被内核击杀），
文件由 `deploy/clickhouse/99-memory-cap.xml` 只读挂载。

## 数据库远程访问

PG `5432` 和 CH HTTP `8123` 只绑定 253 的 `127.0.0.1`，CH native `9000` 不发布。
CH `default` 密码在服务端以 SHA-256 保存，`access_management=0`；客户端从 `.env` 的
`CLICKHOUSE_USER`/`CLICKHOUSE_PASSWORD` 认证。
需要从 Mac 临时直连时使用 SSH 隧道：

```bash
ssh -N -L 15432:127.0.0.1:5432 -L 18123:127.0.0.1:8123 home-debian
# PG: localhost:15432；CH: http://localhost:18123（仍须用户名/密码）
```
