...
> **1a 已落地（2026-07-07，commit 3a15a7f）**：B1/B2/B3/B4/B6/B7/B9 七个旁路
> 收编为 `deactivate_missing_securities` / `enrich_security_identity` /
> `recalculate_price_latest_dates` 三个 API。完成判据修订——grep 豁免清单：
> ① `repair_identity`（设计内，阶段 2）；② `build_companies.py` company_id
> 挂接腿（B10，NULL-only 语义合规，待独立 API 或豁免定案）；③
> `cleanup_unknown_figi.py`（一次性修复工具，置 NULL 属 repair 类 OP，与
> repair_identity 同等特权）。语义定案：B2 旧"整行 AND 守卫"统一为逐列
> COALESCE（并发下部分填充行补齐剩余 NULL 列而非整行跳过，绝不覆盖既有值）；
> enrich 返回 rowcount=1 表示至少一列实际补入（保留 B9 竞态计数语义）；
> B2/B3/B4 事务粒度从批量单事务改为逐行提交（幂等操作，最终状态不变）。
